...
2.  User customizations from `$FLOX_CONFIG_DIR/flox.toml` if set or else
    `$XDG_CONFIG_HOME/flox/flox.toml`.
3.  User customizations from `flox/flox.toml` in any of
    `$XDG_CONFIG_DIRS`.
...
`$FLOX_CONFIG_DIR`. `$FLOX_CONFIG_DIR` and `config_dir` are ignored.
...
active anvironments \* “hide-all”: disables the modification of the
