...
consitituent packages, the attempt is discarded and the environment is
...
[`manfifest-toml(1)`](./manifest.toml.md) for more details on the
available options.
...
Alternatively, an arbitrary Nix flake installable may be specified. See
[`manfifest-toml(1)`](./manifest.toml.md) for more details.
